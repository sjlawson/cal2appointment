 ...
 * @subpackage Components
 */
class ClientpipeModelException extends JModel
{
	/**
...
/**
	...
	function &getData()
	{
		if(JRequest::getVar('task') != 'edit' && JRequest::getVar('task') != 'add') {
				if (empty( $this->_data ))
			{
				$query = ' SELECT * FROM #__cp_exception ORDER BY `exception_id` DESC ';
		...
		// Load the data
		if (empty( $this->_data )) {
			$query = ' SELECT * FROM #__cp_exception '.
					'  WHERE exception_id = '.$this->_id;
	...
	{	
		...
		$data['on_staff_id_csv'] = implode(',',$data['on_staff_id_csv']);
		$data['off_staff_id_csv'] = implode(',',$data['off_staff_id_csv']);
