...
<a href="<?php echo $applink ?>" >Schedule Exceptions</a>
	...
	for ($i=0, $n=count( $this->items ); $i < $n; $i++)	{
		...
		$link 		= JRoute::_( 'index.php?option=com_nucleocal&controller=exception&task=edit&cid[]='. $row->exception_id );
		?>
		<tr class="<?php echo "row$k"; ?>">
			...
			<td>
				<?php
				if($row->slot_id) {
				$dbo->setQuery('SELECT * FROM `#__nc_slot` WHERE `slot_id` = '.$row->slot_id ); 
