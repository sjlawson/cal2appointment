 ...
 * @subpackage Components
 */
class NucleocalModelStaff extends JModel
{
	/**
...
/**
	...
	function &getData()
	{
		if(JRequest::getVar('task') != 'edit' && JRequest::getVar('task') != 'add') {
				if (empty( $this->_data ))
			{
				$query = ' SELECT * FROM #__nc_staff ORDER BY `staff_id` DESC ';
	...
	{	
		...
		$sql = "SELECT * FROM `#__nc_slot_alloc` WHERE `staff_id` = ".$data['staff_id'];
		$dbo->setQuery($sql);
		...
		if((is_array($data['slot_id']) && count($data['slot_id'])) || count($oldslots) ) {
			...
			if(in_array('none',$data['slot_id'])) {
				$dbo->setQuery("DELETE FROM `#__nc_slot_alloc` WHERE `staff_id` = ".$data['staff_id']);
				$dbo->query();
			} else {
				foreach($data['slot_id'] as $selectedSlot) {
					if(!in_array($selectedSlot, $oldslots)) {
						//add new slot alloc record
						$sql = "INSERT INTO `#__nc_slot_alloc` ( `staff_id`, `slot_id` ) VALUES 
				...
				foreach($oldslots as $oldslot ) {
					if(!in_array($oldslot, $data['slot_id'])) {
						...
						$sql = "DELETE FROM `#__nc_slot_alloc` WHERE `slot_id` = $oldslot 
